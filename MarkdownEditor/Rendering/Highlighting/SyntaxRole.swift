...
/// 这里刻意偏离了 `doc/代码高亮方案.md` 的设计（那里建议用 `Range<String.Index>`）：
/// 最终消费方是 `NSMutableAttributedString.addAttribute(_:value:range:)`，
/// 用 `String.Index` 的话每个 token 都要 `NSRange(token.range, in: code)` 换算一次 ——
/// 而高亮是**每敲一个字都要重跑一遍**的热路径，能省就省。
///
